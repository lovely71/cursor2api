...
> ⚠️ **版本说明**：当前版本为 v2.7.7，已包含工具透传模式（passthrough）、工具禁用模式（disabled）、身份泄漏清洗增强和 `tool_choice=any` 引导优化；同时重点修复长 `Write/Edit` 截断续写、OpenAI 流式工具调用恢复，并新增 `degraded` 日志状态与降级原因展示，方便排查“看似成功、实际体验不佳”的请求。
