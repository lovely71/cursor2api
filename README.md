...
> ⭐ **v2.7.8 版本说明**：在 v2.7.7 已有的工具透传模式（passthrough）、工具禁用模式（disabled）、身份泄漏清洗增强、长 `Write/Edit` 截断续写恢复与 `degraded` 日志诊断基础上，进一步新增上下文压力膨胀（Context Pressure Inflation）、自适应历史预算、工具结果智能截断三大防截断机制。默认关闭，按需开启。
